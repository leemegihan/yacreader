...
현재 기능과 남은 범위는 [현재 기능과 다음 단계](manga-library-roadmap.md)를 먼저 참고한다.
...
- Validated with the paired-credit and bounded-probe changes at code f69829e16a78ae7754ca1271f286507341c65928; final evidence is recorded below. Real text, paths, candidates and measurements remain local.
...
- Add synthetic worker and C++ regressions for delimiter limits, role order, adjacent-name geometry, incomplete/ambiguous pairs and explicit review. Fixed-input actual CPU/GPU comparison and final package validation completed at code f69829e16a78ae7754ca1271f286507341c65928; see the evidence below. Real text and review answers remain private.
...
## 2026-09-09 — final paired-credit, banner and bounded-probe validation

- Code f69829e16a78ae7754ca1271f286507341c65928: [Windows validation](https://github.com/leemegihan/yacreader/actions/runs/34316525395) passed all 12 CTest suites, 17 Python worker tests, required staged neural checks (8 passed), installed OCR/candidate checks (21 passed), GUI startup and downloaded-artifact checks (4 passed). Prerequisite-stage skips remain separate from required package checks.
- [General build](https://github.com/leemegihan/yacreader/actions/runs/34316528203) passed formatting, Windows x64/ARM64, both Linux backends, macOS Universal and Docker amd64/arm64. Fork signing, notarization and publishing steps were skipped as configured; this is not a signed release.
- A fresh independent local runtime passed 19 targeted C++ tests plus initialization/cleanup. The ZIP matched its published SHA256 and included hidden PaddleX version metadata. All nine model files matched the actual-test copy. LF/CRLF worker hashes were recorded separately; newline-normalized source and Python AST matched without rewriting the packaged worker.
- Fixed-input GPU/CPU8 comparison retained each device's primary readings, old alternatives and candidates. Separate CPU4/16 runs reproduced the same final metadata candidate sets. Parser-only replay isolated the joined-banner change from the worker-dependent ordered-credit recovery. Candidate agreement is not overall transcription or identity accuracy.
- The C++ wrapper separately passed missing-addon CPU fallback and actual GPU inference after adding a private validation copy of the optional runtime. A real-page process using exact packaged worker bytes appeared in the NVIDIA process list. These checks are distinct from synthetic OCR and excluded from the paired timing matrix.
- New compiled page probes reproduced original selected-entry hashes and prepared pixels. Archive and previously derived image-folder ordering agreed. Derived folders are parity fixtures, not additional independently discovered original folder works.
- A separate front/back-six audit stayed within the existing fixed works and retained the original baseline pages. Its diagnostic and candidate findings remain local and are excluded from the paired CPU/GPU benchmark. Saved diagnostics provide a review index; role keywords in prose are not identity labels and a missing keyword does not establish detection success.
- Both independently extracted GUI apps created Qt windows and remained running during isolated startup checks; only those check processes were closed. Installation ran in isolated CI. Local validation did not execute installer upgrade hooks or alter the current installation or operating database.
- Initial banner failures identified overly broad Unicode script classes and an outdated direct-lookup expectation. Explicit character ranges, separator-count restrictions and review-boundary regressions passed in the final artifact.
- Real media, paths, OCR, reviewed text/roles, candidate details and timings remain outside Git/CI. This documentation-only record does not change the tested application code. No whole-library processing or unreviewed metadata writes were performed.
...
## 2026-09-12 전체 화면 검사와 선택 작품 세션 검증

- `723cfc15` Windows34693224819 전체 빌드·설치 검증 완료: CTest13,
  worker21, staged8, installed21, downloaded147, store25, synthetic fault18.
- `17a13926` Windows34694808798도 전체 성공: downloaded149, 나머지
  필수 검사 수 유지. 실제 합성 CPU OCR을 한 페이지 기록 뒤 중단하고,
  화면에서 이어 읽기 및 새 창에서 결과 복원하는 검사가 포함됐다.
- 정확한 `17a13926` 전체 로컬 패키지의 해시·진단 실행본·모델9개를
  확인했다. 새 native36, store25, 실제 CPU5 검사가 모두 통과했다.
  실제 GPU와 고정16개 복사본의 새 화면 경로 비교는 진행 중이다.
- 별도 프로필의 전체 앱에서 합성 이미지 폴더와 압축파일을 생성·표시했다.
  가로 표지의 왼쪽 글자가 Grid의 중앙 자르기로 사라지는 표시 문제를 확인해
  Grid와 이어 읽기 헤더의 가로 표지만 전체 맞춤으로 바꾼다. 세로 표지와
  카드 크기는 유지한다. 이 표시 수정의 새 패키지 화면 검증은 아직 남았다.
- 원본·운영DB·현재 설치 앱은 변경하지 않았다. 실제 작품 복사본과 그
  시험DB/결과는 개인 시험 폴더에만 저장하며 Git/CI에 넣지 않는다.


## 2026-09-12 CPU 선택 시 불필요한 GPU 런타임 검사 분리

- 선택 작품의 저장 결과를 다시 여는 경로를 확인하면서 CPU 설정에서도
  선택 NVIDIA 런타임 전체를 읽고 검사하는 것을 발견했다. CPU 실행이
  손상된 추가 GPU 설치본 때문에 실패하고, GPU 파일 변경이 CPU 작업
  식별까지 바꾸는 원인이기도 했다.
- CPU를 명시한 작업은 공용 파일·CPU 런타임·모델을 계속 새로 검증하며,
  사용하지 않는 GPU 추가 설치본은 작업 환경에 포함하지 않는다. GPU
  요청의 엄격한 추가 설치본 검사와 CPU 전환을 위한 CPU 검사는 유지한다.
- 합성 파일 변경 검사에 CPU 선택의 추가 설치본 독립성과 필수 CPU/모델
  변경 검출을 추가했다. 새 Windows 빌드와 실제 지연 시간 개선 검증은
  아직 진행 전이며, 기존 통과 결과로 이 수정을 검증했다고 보지 않는다.


## 2026-09-12 검사 창 원본 미리보기의 초기 폭

별도 전체 앱에서 합성 압축작품의 검사 창을 처음 열면 원본 미리보기가
가늘게 눌려 작가·제목을 읽기 어려웠다. 분할선을 옮기면 같은 이미지가
정상 표시되는 것을 확인했다. 초기 분할 크기를 페이지 목록150, 원본500,
결과400의 비율로 지정하고 패널이 완전히 접히지 않게 한다. 사용자가
분할선을 조절할 수 있고 원본 픽셀·OCR 입력·영역 좌표는 변경하지 않는다.
새 패키지의 화면 검증은 아직 남아 있다.


## 2026-09-12 다시 열 때 읽기 설정 유지

앱을 다시 열면 기본 모델로 돌아가 저장 결과 버튼이 비활성화되는 불편을
줄이기 위해 앞뒤 장수·언어·모델·처리 장치·자동 조회 선택을 프로필에
기억한다. 사용자가 제어한 설정만 기록하고 제목·작가·서클·덮어쓰기와
작품별 영상 보정은 기록하지 않는다. 미설치 추가 런타임/잘못된 설정은
사용 가능한 기본 선택으로 돌아간다. 프로필 격리·실제 선택 신호 후 재열기·
미설치/잘못된 선택 처리 검사를 추가했으며 새 Windows 검증은 대기 중이다.


## 2026-09-13 CPU 실행 분리와 읽기 설정의 전체 패키지 검증

- `3df36b34`의 Windows 빌드·설치 검증 `34699238925`가 완료됐다.
  해시를 확인한 진단 기록에서 설치 후 OCR21, 다운로드 검사150, 작업DB25,
  모의 오류18, staged neural8 모두 실패·건너뜀 없이 통과했다. 초기 빌드의
  선택 검사 건너뜀은 필수 패키지 검사와 별도로 기록한다.
- 정확한 전체 실행본과 최종 진단 실행 파일, 워커 소스 및 모델9개를 대조했다.
  로컬 native38/store25, 실제 합성 CPU 전환5/GPU5, GPU 추가 실행 환경이
  있는 상태의 명시적 CPU4, 모의 오류18이 모두 통과했다. GPU 작업 프로세스
  다섯 개를 NVIDIA에서 독립 관측했고 명시적 CPU 작업은 관측되지 않았다.
  모의 오류 검사 뒤 실제 워커를 복원했다. 기존 설치 앱을 교체하지 않았다.
- 초기 진단 실행본의 composed-runtime38 통과와 정확한 전체 실행본의 통과를
  구분한다. 별도 합성 압축/폴더 작품의 Grid 가로 표지와 초기 미리보기 폭,
  앱을 종료하고 다시 실행한 뒤의 읽기 설정 유지를 화면에서 확인했다. 합성
  압축작품의 페이지 이동과 라이브러리 현재 페이지 반영, 이어 읽기 가로
  표지도 확인했다. 위 합성 건수를 실제 작품 정확도로 계산하지 않는다.
- 개인 표본·경로·이미지·OCR 결과·정답은 비공개 기록에만 보관한다. 검토가
  필요한 역할과 이름은 자동 확정하지 않는다. 전체 라이브러리 실행이나
  일괄 저장은 시작하지 않았다.


## 2026-09-13 표지 흐름 표시와 비공개 지연 진단

- 기존 정보 보기의 표지 흐름에서는 가로 표지 가장자리가 여전히 잘렸다.
  Grid와 같은 가로 이미지 맞춤 표시를 적용한다. 배경 흐림용 이미지는
  기존 표시를 유지한다. 이 추가 변경의 빌드와 화면 검증은 아직 대기 중이다.
- 비공개 선택 작품 진단에 상태 변화 기록과 명시적 진단 전용 QtTest 시간
  제한을 추가한다. OCR 완료 후 결과 재열기까지 포함하는 함수의 기본
  제한 때문에 생긴 실패를 원래 기록과 분리하고, 다음 지연 때 마지막
  확인 단계를 남기기 위한 것이다. 일반 앱/워커 제한은 바꾸지 않는다.
  상태 표본 기록은 순수 OCR 속도 측정이 아니며 실제 결과는 공개하지 않는다.


## 2026-09-13 실행 전 확인 단계의 진행 표시

실행 환경 파일을 검사하는 동안 한 줄의 영문 안내만 오래 유지되어 멈춘 것처럼
보일 수 있었다. 앱/모델/공용 파일/CPU/NVIDIA/최종 변경 확인 단계를 한국어로
표시하고, 확인한 파일 수와 읽은 용량을 최대 초당4회 갱신한다. 원본·모델·실행
파일의 해시 검증은 생략하지 않으며 이 변경을 속도 향상으로 계산하지 않는다.
진행 콜백에서 중지하거나 마지막 확인 직전에 파일이 변경되면 결과를 반환하지
않는 검사를 추가한다. 콜백은 최종 안정성 확인 뒤에 호출하지 않는다.
비공개 상태 추적은 파일 수만 바뀌는 안내를 하나의 단계로 묶어 기록한다.
이 후속 변경의 새 빌드와 실행 검증은 아직 대기 중이다.


## 실행 환경 해시의 동시 읽기 후속 변경

파일 내용 전체를 검사하되 최대16개 읽기 작업으로 나눈다. 폴더 순서대로
작업을 수집하고 결과도 그 순서로 조립하므로 실행 설정과 매니페스트 지문은
읽기 순서에 영향을 받지 않는다. 파일·용량·깊이 제한, 링크 거부, 읽기 전후와
최종 변경 검사는 유지한다. 읽기 오류나 취소 후에는 모든 읽기 작업을 종료한
다음 반환한다. 진행 콜백은 호출 스레드에서만 실행한다.

합성 검사에 순차/병렬 매니페스트 일치, 빈 파일·여러 폴더, 콜백 스레드,
Windows 읽기 잠금 실패와 해제 후 재시도를 추가했다. 이 변경은 아직 새
Windows 빌드 및 실제 실행본 검증 전이며 앱 속도 향상을 확정하지 않는다.


## 실행 준비 중 중지 후 미리보기 보존

격리된 합성 작품 GUI에서 실행 환경 확인 중 중지하면 기존 앞뒤 미리보기가
비워지고 영문 내부 오류가 표시되는 것을 확인했다. 새 원본 정보가 반환되기
전에 취소한 경우 기존 미리보기·선택 페이지·검토 입력과 기존 저장 검증 정보를
유지하고 한국어로 중지 상태를 안내한다. 새 OCR 결과로 취급하거나 외부 검색,
저장을 실행하지 않는다. 정상 페이지 처리 중의 중단·재개 흐름은 유지한다.
미리보기만 있는 경우와 기존 검토 결과가 있는 경우를 각각 검사한다.
이 후속 수정의 새 빌드와 GUI 재검증은 아직 대기 중이다.


## 2026-09-13 준비 중지 수정의 Windows 검증과 완료 직후 중지

`aa8267fc` Windows34731633774의 전체 빌드·설치 검증이 성공했다.
해시를 확인한 진단 기록에서 downloaded152, installed21, staged8, store25,
모의 오류18이 실패·건너뜀 없이 통과했다. 로컬 전체 실행본도 native53/store25와
실제 합성 CPU 전환5를 통과했으며, GPU 및 최종 화면 검증은 진행 중이다.

후속 코드 검토에서 작업 계산이 끝난 뒤 UI 완료 콜백을 받기 전에 중지하면
자동 대조가 시작될 수 있는 경계를 확인했다. 취소 토큰이 설정된 경우 새 자동
조회를 막되 완료 결과와 수동 대조는 유지한다. 작업 스레드를 종료한 뒤 UI
이벤트 전달 전에 중지하는 결정적 합성 검사와 정상 완료 비교를 추가했다.
외부 HTTP를 실행하지 않고 요청 신호만 확인한다. 이 후속 수정의 실행 검증은
아직 대기 중이며 실제 작품에서 관측한 오류로 계산하지 않는다.


## 2026-09-13 정상 중지의 미처리 페이지 표시

정확한 `aa8267fc` 로컬 전체 실행본의 native53/store25/실제 합성 CPU 전환5/
GPU5/명시적 CPU4/모의 오류18이 모두 통과했다. GPU 작업자5개를 독립 관측했고
명시적 CPU 작업은 관측되지 않았다. 워커는 모의 오류 검사 뒤 복원했다.
합성 전체 앱에서 준비 중 중지의 한국어 안내와 미리보기·입력 보존도 확인했다.

정상 중지 결과에서 아직 읽지 않은 페이지가 오류/Cancelled로 표시되고 선택
페이지가 첫 장으로 돌아가는 불편을 확인했다. 정상 Paused 결과의 유효 페이지
마스크를 통해 미처리 페이지 번호를 전달하고, 화면에는 이어 읽기 대기로 표시한다.
빈 OCR 문자열로 완료 여부를 추측하지 않는다. 원래 오류·OCR 결과·캐시·DB
기록은 바꾸지 않고 실제 실패는 계속 오류로 표시한다. 결과 갱신 때 같은
페이지 번호의 선택을 유지하며 새 작품이나 일반 미리보기에는 대기 표시를 남기지 않는다.
세 가지 화면 상태와 기존 세션의 중단·재개에서 미처리 번호를 검사한다.
이 후속 변경의 Windows 및 GUI 재검증은 대기 중이다.

완료 직후 중지 검사 `c1fef139`의 첫 CI에서는 정상 비교 자료의 판권 분류가
빠져 자동 조회가0회인 실패가 있었다. `8eded01c`에서 합성 페이지 분류를
보완했다. 해당 중간 빌드와 이번 통합 빌드의 성공 여부는 따로 기록한다.


## 2026-09-13 발행자 후보와 파일명 제목의 수동 대조 연결

발행자·서클 페이지 후보만 있을 때는 이름 힌트를 유지하면서도 파일명 제목을
검토용으로 제안할 수 있어야 한다. 기존 조건은 모든 종류의 페이지 후보가
파일명 제안을 막아, 제목·작가 없이 발행자만 읽힌 경우 대조 화면을 열지 못할
수 있었다. 직접 대조 동작에서 제목·작가 페이지 후보가 없는 경우만 유일한
파일명 제목을 제안한다. 발행자 정보를 유지하고 검색어 검토를 먼저 거친다.
자동 조회·자동 저장·작가 확정이나 저장 작업의 완료 분류는 바꾸지 않는다.
파일명 없음/충돌, 수동 제목 유지, 페이지 작가 우선, 페이지 제목 충돌을 포함한
6개 합성 검사를 추가했다. 새 검증은 대기 중이다.

이전 통합 `02cc5e1b`의 정확한 초기 진단 실행본은 별도 기존 실행 환경에서
58개 로컬 회귀 검사를 통과했고 이전 진단 파일을 복원했다. 전체 새 실행본의
성공과 구분하며, Windows34734713973의 최종 설치·패키지 검증은 진행 중이다.
