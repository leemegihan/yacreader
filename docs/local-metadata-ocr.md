...
- 영역 OCR은 한 작품에서 선택한 앞뒤 최대 6장을 한 프로세스로 읽으며 탐지/인식 모델을
...
영역 탐지 OCR의 선택 작품 처리에는 별도 작업 DB와 페이지 캐시가 연결되어 있습니다.
페이지 결과를 캐시에 기록한 뒤 작업 DB에 완료 페이지를 남깁니다. 현재 라이브러리 항목,
선택한 앞뒤 페이지, OCR 설정과 실행 파일·모델을 확인한 뒤 저장 결과를 사용합니다.
원본 폴더에 캐시나 OCR 결과를 쓰지 않으며, 시험에서는 별도 설정 폴더를 사용합니다.

정상 중지한 작업은 같은 작품과 설정에서 `저장 결과 열기 / 이어 읽기`로 남은 페이지를
이어 읽습니다. 완료한 작업은 OCR을 실행하지 않고 결과를 다시 엽니다. 자세한 사용법과
검증 범위는 아래의 선택 작품 이어 읽기 설명을 참고하세요. 전체 라이브러리 실행과
비정상 종료 복구는 아직 제공하지 않습니다.

캐시 결과의 시간은 원래 OCR 처리 기록입니다. 결과를 다시 열 때도 원본·환경·캐시를
검증하므로, 이번 열기 시간은 원래 OCR 시간과 별도로 표시합니다.
자세한 구현 범위는 [작업 저장소 문서](ocr-job-store.md)를 참고하세요.

## GPU 일부 페이지 실패 시 재시도

GPU 처리 중 실패하면 정상적으로 읽힌 페이지는 빈 페이지를 포함해 보존하고,
실패하거나 결과가 없는 페이지만 CPU로 한 번 다시 읽습니다. 각 페이지에는 실제
처리 장치가 유지됩니다. 취소했거나 기존 프로세스 정리에 실패하면 재시도하지 않습니다.

페이지 결과가 모두 있어도 실행기가 비정상 종료했다면 실행 오류로 표시합니다.
이를 후보 없음으로 처리하거나 모든 페이지를 다시 읽어 오류를 숨기지 않습니다.
영역 탐지 OCR의 선택 작품 처리는 성공한 페이지를 별도 작업 DB·캐시에 보존합니다.
실패한 작업을 정상 완료로 바꾸지는 않으며, 실패·비정상 종료 상태의 재개는 후속 과제입니다.

중지 요청 후 작업 스레드가 종료될 때까지 실행 버튼을 잠급니다. 다른 작품을 선택하면
이전 작업 정리 후 새 미리보기를 시작하고, 창을 닫으면 대기하던 미리보기도 취소합니다.


## 선택 작품 이어 읽기 연결 — 2026-09-12 로컬 개발본

영역 탐지 OCR의 페이지 읽기에 별도 작업 DB·캐시를 연결하고
`저장 결과 열기 / 이어 읽기` 버튼을 추가했습니다. 같은 작품·앞뒤 페이지·설정이면
정상적으로 중지한 작업의 남은 페이지만 읽고, 완료된 결과는 OCR 없이 복원합니다.
복원할 때 입력한 제목·작가를 유지하며 외부 조회를 다시 자동 실행하지 않습니다.
페이지에 표시되는 시간은 원래 OCR 기록이며 이번 열기 시간과 구분합니다.

실행 중·실패·비정상 종료 기록은 자동 재시작하지 않습니다. 앱 강제 종료 후의
복구는 아직 후속 과제입니다. 저장 버튼은 라이브러리 항목과 선택 페이지가 당시
결과와 같은지 확인한 뒤 기존의 검토 후 저장 흐름을 사용합니다.

같은 작품을 다시 선택한 뒤 언어·모델·CPU/GPU·앞뒤 장수를 이전과 같게 맞추고
`저장 결과 열기 / 이어 읽기`를 누릅니다. 정상 중지/완료 기록이 없거나 자료·설정이
달라졌다는 안내가 나오면 기존 결과를 덮어쓰지 않습니다. 새 조건으로 읽으려면
`페이지 글자 읽기`를 명시적으로 선택합니다. 복원된 후보도 직접 검토한 뒤 저장합니다.

`17a13926`의 Windows 전체 빌드·설치 검증과 정확한 전체 로컬 패키지의 native36,
store25, 실제 합성 CPU5/GPU5 검사를 통과했습니다. 합성 자료에서 정상 중지 후
이어 읽기와 새 검사 창의 결과 복원을 확인했고, GPU 검사 작업 프로세스 다섯 개를
NVIDIA에서 관측했습니다. 고정 표본의 실제 장치 비교 결과는 비공개 기록으로 별도 관리하며,
합성 검사를 실제 작품 정답률이나 모든 GUI 동작의 검증으로 간주하지 않습니다.


읽기 장수·언어·모델·CPU/GPU와 자동 외부 조회 선택은 해당 앱 프로필에 기억합니다.
창을 다시 열어도 이 선택을 유지합니다. 추가 OCR 실행 환경이 없어졌다면 사용 가능한
기본 모델/CPU로 돌아가므로 실행 전 표시된 설정을 확인합니다. 제목·작가·서클 입력,
덮어쓰기 및 회전/반전 같은 작품별 보정은 기억하지 않습니다. 보정을 사용한 저장 작업은
이전과 같은 보정을 직접 맞춰야 합니다. 이 설정 기억 변경은 `3df36b34`의 전체 Windows 빌드·설치 검증과 정확한 전체 로컬 실행본의 회귀 검사를 통과했습니다. 별도 합성 압축/폴더 작품에서 넓어진 미리보기와 실제 앱 재실행 후 설정 유지도 확인했습니다.


## 발행자·서클만 읽힌 작품의 파일명 대조

페이지에서 발행자·서클만 읽히고 제목·작가가 없더라도 파일명 제목 힌트가
하나라면 직접 `메타데이터 대조…`를 눌러 검토 화면에 함께 가져올 수 있습니다.
발행자·서클 후보는 그대로 참고 정보로 유지하며 개인 작가로 바꾸지 않습니다.
이 경로는 검색어 검토 화면을 먼저 열고 외부 조회와 DB 저장을 자동 실행하지
않습니다. 확인한 제목 입력칸을 파일명으로 자동 채우지도 않습니다.

사용자가 입력한 제목은 유지하고, 제목·작가 페이지 후보가 있는 경우는 기존
후보 검토를 우선합니다. 파일명 힌트가 없거나 여러 개면 임의로 선택하지 않습니다.
이 후속 변경의 새 Windows 및 실행본 검증은 아직 대기 중입니다.
