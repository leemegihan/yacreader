...
초기 검증은 별도로 추출한 실행본과 테스트 설정·DB에서 진행한다. 현재 설치 앱이나
운영 DB를 교체하지 않는 검증 방법은 `local-ocr-validation.md`를 따른다.
아래 설치 안내는 나중에 사용자가 운영 버전 교체를 선택했을 때의 일반 절차다.
...
3. 기본 앞 3장·뒤 3장의 미리보기를 확인한다. 각 1~3장 범위에서 줄일 수 있다.
   이 범위에서 후보가 없으면 페이지를 늘리지 않고 파일명·외부 대조로 진행한다.
...
일반 검사 화면은 앞뒤 최대 3장씩만 읽습니다. 선택한 페이지의 OCR이 오류 없이 끝났는데
제목·작가·발행자/서클 후보가 없으면 페이지 수를 늘리지 않고 파일명·이름 힌트로
대조하는 경로를 안내합니다. 이는 다음 처리 방법을 정하는 기준이며 작품 전체에
단서가 없다는 검증 결과가 아닙니다. 취소·미리보기·읽기 실패는 후보 없음과 구분합니다.

파일명 대조 버튼을 누르면 정리한 제목 힌트를 검토 화면으로 가져옵니다. 확인한 제목·작가
입력칸은 자동으로 채우지 않고, 외부 요청과 DB 저장도 자동 실행하지 않습니다. 사용자가
이미 입력한 값은 유지합니다. 검토 화면에서 검색어를 고친 뒤 조회할 수 있으며,
지원하는 작품 링크를 알고 있다면 직접 붙여 넣어 메타데이터를 대조할 수도 있습니다.

여러 선행 대괄호에서 일반적인 번역·언어·형식 표시는 제외하고 제목을 분리합니다.
설명되지 않은 대괄호가 하나면 낮은 신뢰의 이름 힌트로 남기지만, 여러 개면 분류 태그와
서클/개인을 구분할 수 없어 작가를 임의 선택하지 않습니다. 원래 파일명은 계속 표시됩니다.
검색 결과와 이름 힌트는 근거를 대조해야 하며 정답으로 자동 승격하지 않습니다.

대조 화면의 이름 힌트 3칸에서 실제 검색에 쓸 값을 확인하고 수정·삭제할 수 있습니다. 힌트를 바꾸면 이전 대조 결과를 지우며, 작품을 전환하면 이전 힌트도 초기화합니다.
