...
4. 언어와 모델을 선택하고 `페이지 글자 읽기`를 누른다. 기본값은 일본어+영어,
   정밀 모델이다. 한국어 번역본이면 한국어+영어로 바꾼다.
...
`메타데이터 대조…`는 OCR 제목·작가와 페이지 수를 대조 화면에 전달한다.
그 화면에서 `Search`를 누를 때만 선택한 서비스로 조회한다. E-Hentai는
...
조회한 제목·작가·태그가 검토 화면에 자동 입력되며, `Apply`를 눌러야 DB에 저장한다.
...
이번 정밀 OCR·외부 대조 변경의 Windows 결과는 새 Actions 실행에서 확인해야 한다.
