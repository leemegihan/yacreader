...
선택한 제품 워커로 실제 NVIDIA 실행과 CPU 4·8·16스레드 비교를 별도로 수행했다.
앱의 공통 페이지 디코딩·전처리를 사용한 동일 표본을 비교했으며, 위 합성 자료의
엔진 간 비교와 구분한다. 실제 표본, OCR 결과, 후보, 정답과 상세 측정 기록은 로컬에만 둔다.

장치 간 문자열이나 후보의 일치를 실제 정답 정확도로 취급하지 않는다. 탐지 상자,
배제된 문자 후보, 역할 연결 결과를 구분하는 선택적 진단과 데이터베이스를 열지 않는
페이지/후보 검사 절차는 [로컬 검증 안내](local-ocr-validation.md)에 정리했다.
